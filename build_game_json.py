...
def build_game_json() -> dict:
    ...
    return {
        ...
        "customHelp": (
            ...
            "become a Rod (mana). Drag any hand card to Rods and flip it face-down (right-click "
            "-> Flip / Hide).\n"
            ...
            "(4) NEW CYCLE (end turn / spacebar): all flopped cards in Rods + Huddle auto-unflop. "
            "Both players' turn ends together (shared turn).\n"
            "(5) VIBE CHECK at cycle end: compare highest-vibe Penguin between players. Winner "
            "claims the Baron's Favor. Winner MANUALLY moves the top of their deck to Rods "
            "(drag); loser MANUALLY draws the top card to Hand. (No auto-draw - this is by design.)\n"
            "(6) BARON'S FAVOR: right-click anywhere -> Create Tokens -> 'Baron' to spawn the "
            "token. Drag into your Baron zone (gold-bordered slot, right of Huddle). To PASS to "
            "the opponent: right-click the token -> 'Give to player' - it auto-lands in their "
            "Baron zone.\n"
            "(7) PERMANENT COUNT: track manually - bump your player counter (+/- buttons at your "
            "nameplate) for each Rod + Penguin/Relic in your Huddle. Tokens don't count.\n"
            "(8) WIN CONDITION: at the START of a cycle, if you hold the Baron's Favor AND your "
            "permanent count is 15 or more, you win.\n"
            "(9) PRIORITY: no built-in pass button - say 'pass' in chat to give priority. End "
            "turn (spacebar) ends the cycle for both players."
        ),
        "cardRotation": "90",
        "cards": {
